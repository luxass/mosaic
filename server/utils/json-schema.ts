...
export const PROJECT_SCHEMA = z.object({
  priority: z
    .number({
      description:
        "the priority of the project. the higher the number, the higher position the project will have on `luxass.dev`.",
    })
    .default(10),
...
export const NPM_SCHEMA = z.object({
  ...
  name: z
    .string({
      description:
        "the name of the npm package. by default the `name` will be auto-inferred from `package.json`",
    })
    .optional(),
...
export const README_SCHEMA = z.object({
  ...
  path: z
    .string({
      description:
        "the path to the readme file. by default the `path` will be auto-inferred from the repository.",
    })
    .optional(),
});
...
export const WEBSITE_SCHEMA = z.object({
  ...
  description: z
    .string({
      description:
        "The description to set in the meta description tag. If not provided, the description will be inferred from `project.description`",
    })
    .optional(),
...
export const WORKSPACE_SCHEMA = z.object({
  ...
  overrides: z
    .record(
      BASE_MOSAIC_SCHEMA.omit({ project: true }).merge(
        z.object({
          project: PROJECT_SCHEMA.merge(
            z.object({
              name: z.string({
...
export const MOSAIC_SCHEMA = BASE_MOSAIC_SCHEMA.merge(
  z.object({
    project: PROJECT_SCHEMA.merge(
      z.object({
        name: z.string({
