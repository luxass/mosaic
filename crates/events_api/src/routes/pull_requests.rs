...
#[utoipa::path(
  get,
  path = "/api/v1/events/pull-requests",
  tag = TAG,
  responses(
...
#[debug_handler]
pub async fn get_pull_request_events(State(state): State<AppState>) -> Result<Json<Vec<MosaicEvent>>, ApiErrorResponse> {
  match state.github.get_user_events("luxass").await {
    Ok(events) => {
      let mapped_events = events
        .into_iter()
        .filter(|event| event.r#type == "PullRequestEvent")
