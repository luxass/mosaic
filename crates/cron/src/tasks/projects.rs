...
pub async fn update_projects(data: &Data<AppState>) -> Result<(), AppError> {
    ...
    let repositories = nodes.iter().filter(|repo| {
        if let Some(repo) = repo {
            !repo.is_fork
                ...
                && !ignored_projects.contains(&repo.name)
        } else {
            false
        }
    });
    ...
    for repository in repositories.into_iter().filter_map(|repo| repo.as_ref()) {
    ...
    let repositories_with_config: Vec<_> = resolved_configs
        .into_iter()
        .filter_map(|(repo, config)| config.map(|config| (repo, config)))
        .collect();
