...
pub async fn update_projects(data: &Data<AppState>) -> Result<(), AppError> {
    ...
    let repositories = nodes.iter().filter(|repo| {
        if let Some(repo) = repo {
            !repo.is_fork && !repo.is_archived && !ignored.contains(&repo.name_with_owner) && !ignored.contains(&repo.name_with_owner.split("/").nth(1).unwrap().to_string())
        } else {
            false
        }
    });
...
async fn fetch_ignored_projects() -> Result<Vec<String>, AppError> {
    let response = reqwest::get("https://raw.githubusercontent.com/luxass/luxass/main/.github/mosaic/.mosaicignore").await?;

    if response.status().is_success() {
        let ignored_projects = response.text().await?;
        Ok(ignored_projects.lines().filter(|s| !s.is_empty() || s.starts_with("#")).map(|s| s.to_string()).collect())
    } else {
        Err(AppError::TaskError("Failed to fetch ignored projects".to_string()))
    }
}
